...
def main():
    ...
    parser.add_argument('--machines', '-m', type=int, default=-1, help='how many machines to shard the tests over. must also provide individual shard index. Default=-1.')
    parser.add_argument('--rpcgroup', '-r', type=int, default=-1, help='individual shard index. must also provide how many machines to shard the tests over. Default=-1.')
    ...
    if((args.rpcgroup == -1 and args.mahines != -1) or (args.rpcgroup ==-1 and args.rpcmachines != -1)):
        print("ERROR: Please use both -m and -r options when using parallel rpc_groups.")
        sys.exit(0)
    if( args.rpcgroup != -1 and args.machines != -1 and args.rpcgroup < args.machines ):
